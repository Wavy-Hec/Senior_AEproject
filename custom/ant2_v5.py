...
class SuperAE():
    def __init__(self) -> None:
        arr = np.load("AntObservations.npy")
        arr = np.delete(arr,slice(27,111),1)

        #new data (trained ant)
        ...
        self.AE = SimpleAutoencoder(latent_dimensions, input_data_shape)


        #losses.MeanSquaredError()
        self.AE.compile(optimizer='adam', loss=losses.MeanAbsoluteError())

        self.AE = load_model('./simple_AE2')
        print("ready to go!")
        ...
        self.AE.encoder.trainable = False
        self.AE.decoder.trainable = False

class Ant2Env(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        healthy_reward=0.1, # ERIK - original reward was 1.0, we can tune
        ...
        numFeatures=27, #squeeze from 26 and so on manaully
        ...
        **kwargs
        ...
        # cancel squeezer if numFeatures is 27
        if numFeatures == 27:
            squeezer = False
        ...
        if kbest_file:
            ...
            self.num_features_kbest = kbest_num_features
        else:
            self.kbest = None
            self.num_features_kbest = None
    ...
    def terminated(self):
        ...
        terminated = terminated or (self.epi_steps >= self.max_steps)
        return terminated
