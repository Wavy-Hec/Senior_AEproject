...
class Human2Env(MujocoEnv, utils.EzPickle):
    """
    ### Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        squeezerType='AE',
        numFeatures=376, #squeeze from 376 and so on manaully
        **kwargs
    ):
        ...
        # cancel squeezer if numFeatures is 376
        if numFeatures == 376:
            squeezer = False
        ...
        if squeezer:
            if squeezerType == "AE":
                print("squeezing!")
                self.squeezer = load_model(f'Human_Encoder/HAE_990Rwrd_500E_{numFeatures}.keras')#HAE for differnt AE
                obs_shape = numFeatures
           # elif .. 'PCA' remove for now as just setting up AE
        ...
        if exclude_current_positions_from_observation:
            ...
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(376,), dtype=np.float64 #shape could be wrong
            )
...
model.learn(total_timesteps=10000, tb_log_name='test0')
